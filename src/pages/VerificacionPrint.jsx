  ...
  return (
    <>
      ...
      <div className="print-container">
        <div className="print-page uppercase-print">
          {/* Encabezado */}
          ...
          <div className="section-wrapper">
            ...
            <div className="section-box">
                {/* Aquí la tabla no será editable, será una visualización */}
                <table className="table table-bordered table-sm">
                    ...
                    <tbody>
                        {filas.map(fila => (
                            <tr key={fila.id}>
                                <td>{fila.numeroToma}</td>
                                <td>{fila.sector}</td>
                                <td>{fila.descripcionTerreno}</td>
                                <td>{fila.usoPuestaTierra}</td>
                                <td>{fila.esquemaConexion}</td>
                                <td>{fila.valorResistencia}</td>
                                <td>{fila.cumple}</td>
                                <td>{fila.continuidad}</td>
                                <td>{fila.capacidadCarga}</td>
                                <td>{fila.proteccionContactos}</td>
                                <td>{fila.desconexionAutomatica}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
          ...
          {/* Referencias van en la misma página */}
          <div className="section-wrapper">
              <h2 className="section-title">Referencias de columnas</h2>
              <table className="table table-bordered table-sm small">
                <tbody>
                  <tr><td><b>(22) N° de toma de tierra:</b></td><td>Indicar mediante un número la toma a tierra donde se realiza la medición, el cual deberá coincidir con el del plano o croquis que se adjunta a la medición.</td></tr>
                  <tr><td><b>(23) Sector:</b></td><td>Indicar el sector o la sección dentro de la empresa donde se realiza la medición.</td></tr>
                  <tr><td><b>(24) Descripción del terreno:</b></td><td>Indicar o describir la condición del terreno al momento de la medición (lecho seco, arenoso seco o húmedo, lluvias recientes, turba, limo, pantanoso, etc.).</td></tr>
                  <tr><td><b>(25) Uso de la puesta a tierra:</b></td><td>Indicar el uso habitual, como puesta a tierra del neutro de transformador, seguridad de masas, protección de equipos electrónicos, informática, iluminación, pararrayos, otros.</td></tr>
                  <tr><td><b>(26) Esquema de conexión a tierra:</b></td><td>Indicar cuál es el esquema utilizado (TT / TN-S / TN-C / TN-C-S / IT).</td></tr>
                  <tr><td><b>(27) Valor obtenido (Ω):</b></td><td>Indicar el valor en Ohm obtenido en la medición de resistencia de puesta a tierra de las masas.</td></tr>
                  <tr><td><b>(28) Cumple con reglamentación:</b></td><td>Indicar si el resultado cumple con la Reglamentación de la Asociación Argentina de Electrotécnicos para instalaciones eléctricas en inmuebles.</td></tr>
                  ...
                  <tr><td><b>(32) Desconexión automática:</b></td><td>Indicar si el dispositivo de protección puede desconectar automáticamente el circuito dentro de los tiempos establecidos por la normativa.</td></tr>
                </tbody>
              </table>
          </div>
